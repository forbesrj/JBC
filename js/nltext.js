angular.module('vr.directives.nlForm.text',[])
    .directive('nlText', function(){
        return {
            ...
            template:
                ...
                '<input type="text" placeholder="{{ placeholder }}" name="{{ name }}" ng-model="value" ng-click="textClick()" ng-required="required"/>' +
                '<button class="nl-field-go" ng-click="close()">Go</button>' +
                '</li>' +
                '<li class="nl-ti-example" ng-show="showSubline()">For example: <em>{{sublineSplit[0]}}</em> or <em>{{sublineSplit[1]}}</em></li>' +
                '</ul>' +
                '</div>',
    ...
    .controller('nlTextCtrl',['$scope', '$sce', function($scope){
        $scope.sublineSplit = $scope.subline.split(';');
